    ...
    try {
        ...
        if (Array.isArray(msg.SigList)) {
          ...
          for (const signal of msg.SigList) {
            try {
              ...
              // Format signal line
              BO_SG_Content += ` SG_ ${signal.Name} : ${StartBitHndl}|${signal.Length}@${ByteOrderStr}${DataType}(${
                signal.Factor},${signal.Offset}) [${signal.PhyMin}|${signal.PhyMax}] "${Unit}" ${Receiver}${eol}`;
            } catch (error) {
      ...
      CANMsgInfo.MsgList.forEach(msg => {
        msg.SigList.forEach(signal => {
          if (signal.InitValue && signal.Factor && signal.Offset) {
            let Ini_Val_Num;
